...
class EmployeeService {
    constructor() {
        ...
        this.getAllEmployees = (filter, search, offset, limit, sort, order) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (search) {
                ...
                query += ` where (email like "%${search}%" or full_name like "%${search}%" or dp.name like "%${search}%")`;
                if (filter) {
        ...
        this.getAllEmployeesAssessment = (userData, filter, search, offset, limit, sort, order) => __awaiter(this, void 0, void 0, function* () {
            ...
            const res1 = yield this.sequelize.query(queryCount);
            const res2 = yield this.sequelize.query(query);
            const res3 = yield this.sequelize.query(query3);
            const total = (res1 && res1[0] && res1[0][0] && res1[0][0].total) ? +res1[0][0].total : 0;
            const employeesList = (res2 && res2[0]) ? res2[0] : [];
            const surveysList = (res3 && res3[0]) ? res3[0] : [];
            if (employeesList.length) {
                for (const emp of employeesList) {
                    ...
                    const res4 = yield this.sequelize.query(queryEmpCounts);
                    const res5 = yield this.sequelize.query(queryEmps);
                    const totalEmp = (res4 && res4[0] && res4[0][0] && res4[0][0].total) ? +res4[0][0].total : 0;
                    const empsList = (res5 && res5[0]) ? res5[0] : [];
                    ...
                    if (empsList.length) {
                        ...
                        for (const empSub of empsList) {
                            const item = {
                                title: common_1.Common.convertFullname(empSub.full_name),
                                id: empSub.id,
                                surveys: []
                            };
                            for (const survey of surveysList) {
                                if (empSub.id === emp.employee_id) {
                                    ...
                                    const res6 = yield this.sequelize.query(queryAssessment, { raw: true });
                                    let data = [];
                                    ...
                                    if (data.length < 1) {
                                        ...
                                        const findData = data.find((ss) => ss.by === empSub.id);
                                        if (findData) {
                                            item.surveys.push({
                                                survey: survey.name,
        ...
        this.getAssessmentSurvey = (userData, assessmentYear) => __awaiter(this, void 0, void 0, function* () {
            ...
            const res1 = yield this.sequelize.query(query1);
            const res2 = yield this.sequelize.query(query2);
            const res3 = yield this.sequelize.query(queryEmps);
            const infoList = (res1 && res1[0]) ? res1[0] : [];
            const surveysList = (res2 && res2[0]) ? res2[0] : [];
            const empsList = (res3 && res3[0]) ? res3[0] : [];
            if (infoList.length) {
                for (const info of infoList) {
                    info.assessment_year = assessmentYear;
                    info.assessments = [];
                    info.full_name = common_1.Common.convertFullname(userData.full_name);
                    ...
                    for (const survey of surveysList) {
                        ...
                        for (const emp of empsList) {
                            ...
                            const res4 = yield this.sequelize.query(queryAssessment);
                            let data = [];
                            ...
                            if (data.length) {
                                const recordItem = data.filter((da) => da.by === userId);
                                if (recordItem.length) {
                                    item.users.push({
                                        by: emp.id,
                                        name: common_1.Common.convertFullname(emp.full_name),
        ...
        this.getAssessmentServeyByUserId = (userId, assessmentYear) => __awaiter(this, void 0, void 0, function* () {
            const resEmp = yield this.sequelize.query('select * from dbplus_kpi_lezo.hr_employees where id=' + userId);
            const departmentId = +resEmp[0][0].department_id;
            const position = resEmp[0][0].position;
            const fullname = common_1.Common.convertFullname(resEmp[0][0].full_name);
            const query1 = `
                        SELECT distinct
                            dp.id as department_id, dp.name as department_name, de.id as employee_id, 
                            de.full_name as full_name, de.position
                ...
                for (const info of infoList) {
                    ...
                    for (const survey of surveysList) {
                        ...
                        for (const emp of empsList) {
                            let queryAssessment = `select * from dbplus_kpi_lezo.staff_assessments where employee_id=${emp.id} and survey_id=${survey.id} and assessment_year=${assessmentYear} and assessments like '%"by":${userId}%'`;
                            const res4 = yield this.sequelize.query(queryAssessment);
                            let data = (res4 && res4[0] && res4[0].length && res4[0][0] && res4[0][0].assessments) ? JSON.parse(res4[0][0].assessments) : [];
                            if (data.length) {
                                const recordItem = data.filter((da) => da.by === userId);
                                if (recordItem.length) {
                                    item.users.push({
        ...
        this.getSurveys = (userData, search, offset, limit, sort, order) => __awaiter(this, void 0, void 0, function* () {
        ...
        this.getAssessmentSurveySubmit = (userData, formData) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (formData && formData.length) {
                ...
                try {
                    for (let i = 0; i < formData.length; i++) {
                        ...
                        const res1 = yield this.StaffAssessemnt.findOne({
                            where: {
                        ...
                        if (res1) {
                            const assData = res1['dataValues'];
                            const currAss = assData.assessments ? JSON.parse(assData.assessments) : [];
                            if (currAss.length > 0) {
                                if (currAss.filter((as) => as.by === +userId).length < 1) {
                    ...
                    if (sendCreateData.length) {
                        sendCreateData.forEach((sd) => {
                            const query = 'insert into dbplus_kpi_lezo.staff_assessments(employee_id,assessment_year,survey_id,assessments) values(' +
                                ...
                                '")';
                            promises.push(that.sequelize.query(query));
                        });
                    }
                    if (sendUpdateData.length) {
                        sendUpdateData.forEach((sd, ind) => {
        ...
        this.getEmployeeById = (userId) => __awaiter(this, void 0, void 0, function* () {
        ...
        // Get employee profile
        this.updateEmployee = (employeeId, full_name, email, position, department_id) => __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                const query = `update dbplus_kpi_lezo.hr_employees set full_name='${common_1.Common.convertFullname(full_name)}',  email='${email}', position='${position}', department_id='${department_id}' where id=${employeeId}`;
                this.sequelize.query(query).then(res => {
                    if (res) {
                        resolve('Update employee successful');
                    }
                    else {
                        reject(interfaces_1.ErrorConstants.EMPLOYEE_UPDATE_FAILED);
                    }
                });
            }));
        });
        // Get employee profile
        this.updateSurvey = (surveyId, surveyName, surveyDescription) => __awaiter(this, void 0, void 0, function* () {
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                ...
                this.sequelize.query(query).then(res => {
                    if (res) {
                        ...
                        reject(interfaces_1.ErrorConstants.SURVEY_UPDATE_FAILED);
                    }
                });
            }));
        });
